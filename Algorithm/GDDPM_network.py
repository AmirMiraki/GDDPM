...
class GDDPMTrainingNetwork(nn.Module):
    @validated()
    def __init__(
        self,
        input_size: int,
        ...
        **kwargs,
        ...
        # self.rnn = nn.Linear(input_size,num_cells)
        # self.rnn = nn.TransformerEncoderLayer(d_model=input_size, nhead=4, batch_first=True)
        # self.rnn = Mamba(d_model=input_size,d_state=num_cells)
...
class GDDPMPredictionNetwork(GDDPMTrainingNetwork):
