...
class ResidualBlock(nn.Module):
    def __init__(self, hidden_size, residual_channels, dilation):
        ...
        self.dilated_conv2 =tgnn.GatedGraphConv(2 * residual_channels, 1)
    ...
    def forward(self, x, conditioner, diffusion_step, edge_index, edge_weight):
        diffusion_step = self.diffusion_projection(diffusion_step).unsqueeze(-1)
        conditioner = self.conditioner_projection(conditioner)

        y = x + diffusion_step
        sh = y.shape
        # print(sh,conditioner.shape)
        y = self.dilated_conv(y) + self.dilated_conv2(y.reshape(-1,sh[-2]),edge_index, edge_weight).reshape(sh[0],-1,sh[2]) + conditioner
...
class EpsilonTheta(nn.Module):
    def __init__(
        ...
        if not edge_weight==None:
          self.edge_weights = torch.tensor(edge_weight)
    ...
    def forward(self, inputs, time, cond):
        ...
        for layer in self.residual_layers:
            x, skip_connection = layer(x, cond_up, diffusion_step, self.edge_index.to(x.device),edge_weight=self.edge_weights)
